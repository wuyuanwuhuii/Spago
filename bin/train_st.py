...
def split_graph_data_20_test(graph_rna, graph_atac, test_size=0.2):
    # 
    num_nodes = graph_rna.x.shape[0]
    
    # 
    train_idx, test_idx = train_test_split(range(num_nodes), test_size=test_size, random_state=42)
    ...
    return train_rna, train_atac, test_rna, test_atac,train_idx,test_idx
...
def predict_atac(truth,generator,truth_iter, outdir_name, rna2atac, atac2rna, train_idx, test_idx, valid_idx):
    ...
    #sc_atac_test_dataset.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/mouse_embryo/predict_atac_ours.h5ad")
...
def predict_rna(truth,generator,truth_iter, outdir_name, rna2atac, atac2rna, train_idx, test_idx, valid_idx):
    ...
    #sc_rna_test_dataset.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/mouse_embryo/predict_rna_ours.h5ad")
    print("8888888")
    fig = plot_scatter_with_r(
              truth,
              sc_rna_truth_preds,
...
def main():
    ...
    #sc_rna_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/mouse_mebryo/full_rna.h5ad')
    #sc_atac_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/mouse_mebryo/full_atac.h5ad')
    
    sc_rna_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/mouse_embryo/full_rna.h5ad')
    sc_atac_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/mouse_embryo/full_atac.h5ad')
    
    #genes_to_zero = ["Gene1", "Gene2", "Gene3"] 
    #genes_to_zero = [gene for gene in genes_to_zero if gene in sc_rna_train_dataset.var_names]
    #sc_rna_train_dataset[:, genes_to_zero].X = 0
    ...
    # 3.
    sc.pp.highly_variable_genes(sc_atac_train_dataset_copy, n_top_genes=15000)
    ...
    # 
    sc.pp.highly_variable_genes(adata_temp, n_top_genes=10000)
    ...
    #train_rna,train_atac, test_rna, test_atac, val_rna, val_atac = split_graph_data(graph_rna, graph_atac)
    
    train_rna,train_atac, test_rna, test_atac, train_idx,test_idx = split_graph_data_20_test(graph_rna, graph_atac)
    train_idx = train_idx.numpy()  
    test_idx = test_idx.numpy()   
    train_rna_data = sc_rna_train_dataset[train_idx]
    test_rna_data = sc_rna_train_dataset[test_idx]
    
    train_atac_data = sc_atac_train_dataset[train_idx]
    test_atac_data = sc_atac_train_dataset[test_idx]
    
    # 
    train_rna_data.write('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/mouse_embryo/train_rna.h5ad')
    test_rna_data.write('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/mouse_embryo/test_rna.h5ad')
    
    train_atac_data.write('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/mouse_embryo/train_atac.h5ad')
    test_atac_data.write('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/mouse_embryo/test_atac.h5ad')
    dddddddddd
    ...
    #train_rna_s.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/human_data/train_rna.h5ad")
    #train_atac_s.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/human_data/train_atac.h5ad")
    #test_rna_s.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/human_data/truth_rna.h5ad")
    #test_atac_s.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/human_data/truth_atac.h5ad")
    ...
    #rna2atac = both_GAN_1_new.AffineTransform(16, 256, affine_num=20)
    #atac2rna = both_GAN_1_new.AffineTransform(16, 256, affine_num=20)
...
# Model
    ...
    for h_dim, lr, rand_seed in param_combos:
        ...
        def train_cycle_consistency(generator_A, generator_B, num_epochs, train_iter, test_iter,truth_iter,
                            updaterG_A, updaterG_B, lambda_cycle, discriminator, cuda=True, ISRNA=True):
            # 
            device_rna = 'cpu'
            device_atac = 'cpu'
            ...
            for epoch in range(num_epochs):
                ...
                for i in range(1):
                    ...
                    if(ISRNA) :
                        ...
                        rna_recon_cycle_loss = loss_rna(preds=retval1[train_idx], theta=retval2[train_idx], truth=rna_features[train_idx],)
                        ...
                        #rna_recon_cycle_loss = log_zinb_positive(rna_features, retval1, retval3, retval3).mean()
                        #atac_recon_cycle_loss = loss_bce(y, atac_features)
                        atac_recon_cycle_loss = loss_bce(y[train_idx], atac_features[train_idx])
                ...
                if ((epoch + 1) % 10== 0):
                    if ISRNA:
                        ...
                        predict_rna(truth, generator_B, train_atac, outdir_name,  rna2atac, atac2rna, train_idx, test_idx, valid_idx)
                    else:
                        truth = valid_atac.x#.to(device)
                        predict_atac(truth, generator_B, train_rna, outdir_name, rna2atac, atac2rna, train_idx, test_idx, valid_idx)
                # 
