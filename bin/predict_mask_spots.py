...
def spatial_distance_mask_torch(coordinates, mask_ratio=0.2, min_distance=20):
    
    coords_np = coordinates.cpu().numpy()
    N = coords_np.shape[0]
    mask_num = int(N * mask_ratio)
    
    dist_matrix = pairwise_distances(coords_np, metric='euclidean')
    blocked = np.zeros(N, dtype=bool)
    mask_indices = []
    
    indices = np.random.permutation(N)
    for idx in indices:
        if len(mask_indices) >= mask_num:
            break
        if not blocked[idx]:
            mask_indices.append(idx)
            blocked |= dist_matrix[idx] < min_distance
...
def preprocess_combined_graph(adata, feature_type, k=14, metric='cosine'):
    coordinates = torch.tensor(listpredict_atac_ours_50umnt(adata)
    X = adata.X.tocoo() if hasattr(adata.X, "tocoo") else adata.X
    if issparse(X):
        features = torch.tensor(X.toarray(), dtype=torch.float32)
    else:
...
def preprocess_with_mask(adata, feature_type, k=14, metric='cosine', mask_ratio=0.2, min_distance=20, seed=42):
    np.random.seed(seed)
    data = preprocess_combined_graph(adata, feature_type, k, metric)
    coordinates = data.coordinates
    features = data.x

    mask_idx, keep_idx = spatial_distance_mask_torch(coordinates, mask_ratio, min_distance)
    features_masked = mask_features(features, mask_idx)

    print(f"maks spots numbers: {len(mask_idx)}")
...
def predict_rna(truth,generator,truth_iter, outdir_name, rna2atac, atac2rna, train_idx, test_idx, valid_idx, sc_rna_test_dataset):
    ...
    sc_rna_test_dataset.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p21/predict_rna_ours_50um.h5ad")
    print("8888888")
    #fig = plot_scatter_with_r(
    #          truth,
    #          sc_rna_truth_preds,
...
def main():
    ...
    graph_rna, mask_idx_rna, keep_idx_rna = preprocess_with_mask(sc_rna_train_dataset, feature_type='rna', k=14, metric='cosine', mask_ratio=0.05, min_distance=2, seed=42)
    graph_atac, mask_idx_atac, keep_idx_atac = preprocess_with_mask(sc_atac_train_dataset, feature_type='atac', k=14, metric='cosine', mask_ratio=0.05, min_distance=2, seed=42)
    #print(graph_rna)
    #print(mask_idx_rna)
    print(mask_idx_rna)
    ...
    hidden_channels_2 = 32  # 128  32
    latent_channels_2 = 16# 64  15
