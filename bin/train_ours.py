...
sc_rna_test_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p22/MF_rep1_save.h5ad')
sc_atac_test_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p22/mouse_atac_with_coords.h5ad')
...
def predict_atac(truth,generator,truth_iter, outdir_name):
    ...
    sc_rna_atac_truth_preds,z_rna = predict1(generator,truth_iter)
    ...
    if isinstance(z_rna, torch.Tensor):
        z_rna = z_rna.cpu().numpy()  # Convert z_atac to numpy array if it's a tensor
    print(tt.shape)
    #
    adata_save.X = tt
    
    #sc_atac_test_dataset.obsm['z_rna'] = z_rna  # Add z_atac to the obs as new data
    adata_save.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/E15_brain/predict_atac_brain_ours.h5ad")
...
def predict_rna(truth,generator,truth_iter, outdir_name):
    ...
    sc_rna_truth_preds,z_atac = predict2(generator,truth_iter, rna2atac, atac2rna)
    ...
    if isinstance(z_atac, torch.Tensor):
        z_atac = z_atac.cpu().numpy()  # Convert z_atac to numpy array if it's a tensor
    ...
    #sc_rna_test_dataset.obsm['z_atac'] = z_atac  # Add z_atac to the obs as new data
    #sc_rna_test_dataset.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/E15_brain/predict_rna_ours_e15.h5ad")
    print("8888888")
    fig = plot_scatter_with_r(
              truth,
              sc_rna_truth_preds,
...
def mmd_loss(mu1, mu2, logstd1, logstd2):
    # Implementing MMD loss calculation
    return torch.mean(torch.norm(mu1 - mu2, dim=1) + torch.norm(logstd1 - logstd2, dim=1))
...
def main():
    ...
    sc_rna_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p22/MF_rep1_save.h5ad')
    sc_atac_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p22/mouse_atac_with_coords.h5ad')
    ...
    #sc_rna_train_dataset.obs['X'] = sc_rna_train_dataset.obs['x']
    #sc_rna_train_dataset.obs['Y'] = sc_rna_train_dataset.obs['y']
    ...
    #sc.pp.normalize_total(sc_atac_train_dataset_copy, target_sum=1e4)
    ...
    # 
    #sc.pp.highly_variable_genes(adata_temp, n_top_genes=16977)
    ...
    #train_rna,train_atac, valid_rna, valid_atac, train_idx,test_idx = split_graph_data_20_test(graph_rna, graph_atac)
    #test_rna = train_rna
    ...
    hidden_channels_2 = 128  # 128  32 128
    ...
    for h_dim, lr, rand_seed in param_combos:
        ...
        def train_cycle_consistency(generator_A, generator_B, num_epochs, train_iter, test_iter,truth_iter,
            ...
            for epoch in range(num_epochs):
                ...
                if ((epoch + 1) % 10== 0 and epoch > 400):
                    if ISRNA:
                        truth = valid_rna.x#.to(device)
                        predict_rna(truth, generator_B, valid_atac, outdir_name)
                    else:
                        truth = valid_atac.x#.to(device)
                        predict_atac(truth, generator_B, valid_rna, outdir_name)
                # 
        ...
        truth_atac = valid_atac.x#.to(device_rna)
        predict_atac(truth_atac, generatorATAC,valid_rna, outdir_name)
        
        #truth_rna = valid_rna.x#.to(device_rna)
        #predict_rna(truth_rna, generatorRNA, valid_atac, outdir_name, rna2atac, atac2rna,)
        ...
        #loss1_history, loss2_history, loss1_test_history = train_cycle_consistency(generator_A=generatorATAC, generator_B=generatorRNA, num_epochs=400, train_iter=train_loader, 
