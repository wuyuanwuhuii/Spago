...
#sc_atac_test_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/human_data/human_atac_with_coords.h5ad')
...
def split_graph_data_two(graph_rna, graph_atac, test_size=0.1, val_size=0.2, seed=42):
    np.random.seed(seed)
    num_nodes = graph_rna.x.shape[0]  
    ...
    print(val_idx)
    # Save val_idx as CSV
    val_idx_np = val_idx.numpy()  # Convert to NumPy array
    val_idx_df = pd.DataFrame(val_idx_np, columns=['val_idx'])  # Create DataFrame
    val_idx_df.to_csv('val_idx_20.csv', index=False)  # Save to CSV file
...
def spatial_fps_mask_torch(
    coordinates: torch.Tensor,
    mask_ratio: float = 0.2,
    seed: int = 42,
    init: str = "centroid",  # 'centroid' | 'random' | 'farthest'
):
    ...
    if init == "random":
        ...
        d2_center = ((coords - center) ** 2).sum(axis=1)
        first = int(np.argmin(d2_center)) if init == "centroid" else int(np.argmax(d2_center))
    ...
    # 
    min_d2 = ((coords - coords[first]) ** 2).sum(axis=1)
    min_d2[first] = -1.0  # 
    ...
    for _ in range(M - 1):
        ...
        selected[j] = True
        #
        d2 = ((coords - coords[j]) ** 2).sum(axis=1)
        min_d2 = np.minimum(min_d2, d2)
        min_d2[selected] = -1.0  # 

    mask_idx = np.array(sel_list, dtype=int)
    keep_idx = np.setdiff1d(np.arange(N, dtype=int), mask_idx, assume_unique=False)
    return mask_idx.tolist(), keep_idx.tolist()

def mask_features(features, mask_indices, mask_value=0.0):
    
    features_masked = features.clone()
    features_masked[mask_indices] = mask_value
...
def preprocess_with_mask(adata, feature_type, k=14, metric='cosine',
                         mask_ratio=0.2, seed=42, init='centroid'):
    
    data = preprocess_combined_graph(adata, feature_type, k, metric)
    coordinates, features = data.coordinates, data.x

    # 
    mask_idx, keep_idx = spatial_fps_mask_torch(
        coordinates, mask_ratio=mask_ratio, seed=seed, init=init
    )

    # 
    features_masked = features#mask_features(features, mask_idx)  # 

    print(f"FPS uniform mask -> mask {len(mask_idx)} / {features.shape[0]} "
          f"({100*len(mask_idx)/features.shape[0]:.1f}%), keep {len(keep_idx)}")

    return Data(x=features_masked, edge_index=data.edge_index, coordinates=coordinates), mask_idx, keep_idx
...
def predict_atac(truth,generator,truth_iter, outdir_name, rna2atac, atac2rna, train_idx, test_idx, valid_idx, sc_atac_test_dataset):
    ...
    print("saving data .......")
    print(len(valid_idx))
    sc_atac_test_dataset.X[valid_idx]= tt[valid_idx]
    #print("***********************************")
    #print(valid_idx)
    sc_atac_test_dataset.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p21/predict_atac_ours_50um.h5ad")
    #sc_atac_test_dataset.X = tt
    #sc_atac_test_dataset.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p21/predict_atac_ours_50um_new.h5ad")
    print(truth[train_idx].shape)
    #fig = plot_auroc(
    #    truth,
    ...
    fig = plot_scatter_with_r(
        truth[train_idx],
        sc_rna_atac_truth_preds[train_idx],
        one_to_one=True,
        logscale=False,
...
def predict_rna(truth,generator,truth_iter, outdir_name, rna2atac, atac2rna, train_idx, test_idx, valid_idx, sc_rna_test_dataset):
    ...
    sc_rna_test_dataset.X = tt
    sc_rna_test_dataset.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/human_data/predict_rna_ours_50um.h5ad")
    print("8888888")
    #fig = plot_scatter_with_r(
    #          truth,
    #          sc_rna_truth_preds,
...
def mask_spots(adata, test_fraction=0.2, val_fraction=0.1, seed=42):
    np.random.seed(seed)
    
    n_spots = adata.shape[0]  
    n_test = int(n_spots * test_fraction)  
    
    test_indices = np.random.choice(n_spots, n_test, replace=False)
    remaining_indices = np.setdiff1d(np.arange(n_spots), test_indices)
    n_val = int(len(remaining_indices) * val_fraction)  
    ...
    masked_features[test_indices, :] = 0

    masked_features[val_indices, :] = 0

    masked_adata = anndata.AnnData(X=masked_features, obs=adata.obs.copy(), var=adata.var.copy())

    masked_adata.obs["set"] = "train"  # 
    masked_adata.obs.loc[test_indices, "set"] = "test"  
    masked_adata.obs.loc[val_indices, "set"] = "val"  

    return masked_adata, test_indices, val_indices
...
def main():
    ...
    sc_rna_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p21/full_rna.h5ad')
    sc_atac_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p21/full_atac.h5ad')
 
    #is_original_mask = (sc_rna_train_dataset.obs['source'] == 'original').to_numpy()
    #original_indices = np.where(is_original_mask)[0]
    #print(original_indices)
    #is_not_original_mask = ~is_original_mask
    #is_not_original_indices = np.where(is_not_original_mask)[0]
    ...
    sc_atac_train_dataset.X = sc_atac_train_dataset.X.astype(float)
    sc_rna_train_dataset.X = sc_rna_train_dataset.X.astype(float)
    
    
    #sc_atac_train_dataset_copy = sc_atac_train_dataset.copy()
    #sc.pp.normalize_total(sc_atac_train_dataset_copy,)
    #sc.pp.log1p(sc_atac_train_dataset_copy)
    # 3.
    #sc.pp.highly_variable_genes(sc_atac_train_dataset_copy, n_top_genes=20000)
    #num_hvg = sum(sc_atac_train_dataset_copy.var['highly_variable'])
    # 
    #print(num_hvg)
    
    # 4.
    #sc_atac_train_dataset = sc_atac_train_dataset[:, sc_atac_train_dataset_copy.var['highly_variable']].copy()
    ...
    #sc_rna_train_dataset = sc_rna_train_dataset[:, highly_variable_genes].copy()
    #print(sc_rna_train_dataset)
    ...
    sc_atac_test_dataset = sc_atac_train_dataset
    sc_rna_test_dataset = sc_rna_train_dataset
    ...
    graph_rna, mask_idx_rna, keep_idx_rna = preprocess_with_mask(sc_rna_train_dataset, feature_type='rna', k=14, metric='cosine', mask_ratio=0.2, seed=42,  init='centroid')
    graph_atac, mask_idx_atac, keep_idx_atac = preprocess_with_mask(sc_atac_train_dataset, feature_type= 'atac', k=14, metric='cosine', mask_ratio=0.2, seed=42, init='centroid')
    #print(graph_rna)
    #print(mask_idx_rna)
    print(mask_idx_rna)
    df = pd.DataFrame({'mask_idx_rna': mask_idx_rna})
    df.to_csv('mask_idx_rna.csv', index=False)
    ...
    mask_idx_rna = torch.tensor(mask_idx_rna, dtype=torch.long)
    keep_idx_rna = torch.tensor(keep_idx_rna, dtype=torch.long)

    train_idx = keep_idx_rna
    test_idx = mask_idx_rna
    #idx = np.asarray(mask_idx_rna, dtype=int)
    #n = sc_atac_test_dataset.n_obs
    #print(n)
    
    #idx = np.unique(idx[(idx >= 0) & (idx < n)])
    
    #mask = np.zeros(n, dtype=bool)
    #mask[idx] = True
    #mask = False
    #sc_atac_test_dataset.obs['mask_bool']  = mask
    #print("11111112121212")
    #print(sum(sc_atac_test_dataset.obs['mask_bool']))
    #sc_atac_test_dataset.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/data_p21/predict_atac_orign_mask.h5ad")
    #ddddddd
    print(len(train_idx))
    valid_idx = mask_idx_rna
    train_rna_orign = graph_rna
    train_rna =  graph_rna
    train_atac = graph_atac
    train_atac_orign = graph_atac
    test_rna = graph_rna
    ...
    #train_rna_s.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/mouse_E18_brain/train_rna.h5ad")
    #train_atac_s.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/mouse_E18_brain/train_atac.h5ad")
    #test_rna_s.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/mouse_E18_brain/truth_rna.h5ad")
    #test_atac_s.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/mouse_E18_brain/truth_atac.h5ad")
    #valid_rna_s.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/mouse_E18_brain/valid_rna.h5ad")
    #valid_atac_s.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/mouse_E18_brain/valid_atac.h5ad")
    #sc_rna_train_dataset.write_h5ad("/mnt/5468e/twang/WBT/scMOG-main/scMOG-main/scMOG_code/human_data/full_rna.h5ad")
...
# Model
    ...
    for h_dim, lr, rand_seed in param_combos:
        ...
        def train_vgae(model, optimizer, data, epochs, device, truth_atac, discriminator, updaterD):
            ...
            train_coordinates = data.coordinates[train_idx]
            train_edge_index, _ = subgraph(train_idx, data.edge_index, relabel_nodes=True, num_nodes=data.num_nodes)
            train_y_hat = truth_atac.x[train_idx]
            data_train = Data(x=train_x, edge_index=train_edge_index, coordinates=train_coordinates).to(device)
            data = data.to(device)
            ...
            trainD_losses=[]
            #print(data)
            y_hat = truth_atac.x[train_idx].to(device)
            print(y_hat)
            ...
            for epoch in range(epochs):
                ...
                loss2 = loss_D(y[train_idx], y_hat, discriminator, data_train.edge_index, data_train.coordinates)
                loss2.backward()
                updaterD.step()
                ...
                if epoch % 1 == 0:
                    ...
                    loss = loss_atac_G(y[train_idx], discriminator, data_train.edge_index, data_train.coordinates)
        ...
        def train_vgae_atac(model, optimizer, data, epochs, device, truth_rna, discriminator, updaterD):
            ...
            #print(data)
            y_hat = truth_rna.x[train_idx].to(device)
            print(y_hat)
            ...
            for epoch in range(epochs):
                ...
                loss2 = loss_D(retval1[train_idx], y_hat, discriminator, data_train.edge_index, data_train.coordinates)
                loss2.backward()
                updaterD.step()
                trainD_losses.append(loss2.item())
                for p in discriminator.parameters():
                ...
                if epoch % 1 == 0:
                    ...
                    loss = loss_rna_G(retval1[train_idx], discriminator, data_train.edge_index, data_train.coordinates)
                    #reg_loss = model.recon_loss(adj_pred, graph_nei.to(device))
                    reg_loss = regularization_loss(z, graph_nei, graph_neg)
        ...
        def train_cycle_consistency(generator_A, generator_B, num_epochs, train_iter, test_iter,truth_iter,
                            updaterG_A, updaterG_B, lambda_cycle, discriminator, cuda=True, ISRNA=True, sc_atac_test_dataset = sc_atac_test_dataset, sc_rna_test_dataset = sc_rna_test_dataset):
            # 
            device_rna = 'cuda:1'
            device_atac = 'cuda:1'
            ...
            test_atac.to(device_rna)
            test_rna.to(device_rna)
            train_atac.to(device_rna)
            train_rna.to(device_rna)
            
            train_rna.x[test_idx] = 0
            train_atac.x[test_idx] = 0
            test_rna.x[test_idx] = 0
            test_atac.x[test_idx] = 0
            ...
            for epoch in range(num_epochs):
                ...
                for i in range(1):
                    ...
                    if(ISRNA) :
                        ...
                        rna_recon_cycle_loss_1 = loss_rna(preds=retval1[train_idx], theta=retval2[train_idx], truth=rna_features[train_idx],)
                        rna_recon_cycle_loss, recon, kl = generator_B.compute_elbo(mu_rna[train_idx], logstad_rna[train_idx], rna_recon_cycle_loss_1,)
                        ...
                        #rna_recon_cycle_loss = log_zinb_positive(rna_features, retval1, retval3, retval3).mean()
                        #atac_recon_cycle_loss = loss_bce(y, atac_features)
                        atac_recon_cycle_loss_1 = loss_bce(y[train_idx], atac_features[train_idx]) 
                        #print("kkkkkkk")
                        #print(mu_atac[train_idx].shape)
                        #print(logstd_atac.shape)
                       
                        atac_recon_cycle_loss, recon, kl = generator_B.compute_elbo(mu_atac[train_idx], logstd_atac[train_idx], atac_recon_cycle_loss_1)
                        reg_loss_atac = generator_B.recon_loss(graph_nei, adj_pred_atac.float().detach())
                        kl_atac = generator_B.kl_loss(mu_atac, logstd_atac)
                    ...
                    if ISRNA:
                        ...
                        loss_A =  atac_recon_cycle_loss #+ 1 * ( cycle_loss_atac) + atac_recon_cycle_loss_train *  0.01# + reg_loss_atac * 0.01 + lambda_cycle * ( cycle_loss_atac) #+ kl_atac
                ...
                if ((epoch + 1) % 10== 0 and epoch > 300):
                    if ISRNA:
                        truth = valid_rna.x.to(device)
                        predict_rna(truth, generator_B, train_atac_orign, outdir_name,  rna2atac, atac2rna, train_idx, test_idx, valid_idx, sc_rna_test_dataset)
                    else:
                        truth = valid_atac.x.to(device)
                        predict_atac(truth, generator_B, train_rna_orign, outdir_name, rna2atac, atac2rna, train_idx, test_idx, valid_idx, sc_atac_test_dataset)
                # 
                ...
                if test_iter:
                    ...
                    with torch.no_grad():
                        ...
                        if(ISRNA):
                            ...
                            rna_recon_cycle_loss_test_1 = loss_rna(preds=retval1_test[train_idx], theta=retval2_test[train_idx], truth=rna_features_test[train_idx],)
                            rna_recon_cycle_loss_test , recon, kl= generator_B.compute_elbo(mu_rna_test[train_idx], logstad_rna_test[train_idx], rna_recon_cycle_loss_test_1,)
                            ...
                            atac_recon_cycle_loss_test_1 = loss_bce(y_test[train_idx],atac_features_test[train_idx])  
                            atac_recon_cycle_loss_test, recon, kl = generator_B.compute_elbo(mu_atac_test[train_idx], logstd_atac_test[train_idx], atac_recon_cycle_loss_test_1,)
                        ...
                        if ISRNA:
                            ...
                            loss_test = atac_recon_cycle_loss_test # + 0.01 * (cycle_loss_rna_test) + rna_recon_cycle_loss_test_2  #* 10 + reg_loss_rna * 0.01 + lambda_cycle * (cycle_loss_rna ) #  + kl_rna
                        else :
                            #loss_test = atac_recon_cycle_loss  + reg_loss_atac + lambda_cycle * (cycle_loss_atac) #  + kl_atac
                            loss_test = atac_recon_cycle_loss_test #+ 1 * (cycle_loss_atac_test) + atac_recon_cycle_loss_test_2 *  0.01 #* 0.1 + reg_loss_atac * 0.01 + lambda_cycle * (cycle_loss_atac) #  + kl_atac
        ...
        #
        train_vgae(model = generatorATAC, optimizer = optimizer_atac_1, data = train_rna, epochs = 200, device = 'cuda:4', truth_atac = train_atac,discriminator =  ATACdiscriminator, updaterD = optimizer_D_atac)
        ...
        #train_x = train_rna.x[train_idx]
        #train_coordinates = train_rna.coordinates[train_idx]
        #train_edge_index, _ = subgraph(train_idx, train_rna.edge_index, relabel_nodes=True, num_nodes=train_rna.num_nodes)
        #train_rna = Data(x=train_x, edge_index=train_edge_index, coordinates=train_coordinates)
        #test_rna = train_rna      
            
        #train_x_atac = train_atac.x[train_idx]
        #train_coordinates_atac = train_atac.coordinates[train_idx]
        #train_edge_index_atac, _ = subgraph(train_idx, train_atac.edge_index, relabel_nodes=True, num_nodes=train_atac.num_nodes)
        #train_atac = Data(x=train_x_atac, edge_index=train_edge_index_atac, coordinates=train_coordinates_atac)
        #test_atac = train_atac
        ...
        #test_iter=test_loader,truth_iter = truth_loader,updaterG_A=optimizer_A, updaterG_B=optimizer_B, lambda_cycle=0.01, discriminator = RNAdiscriminator, cuda=True, ISRNA=True, sc_atac_test_dataset = sc_atac_test_dataset, sc_rna_test_dataset= sc_rna_test_dataset)
        ...
        test_iter=test_loader,truth_iter = truth_loader,updaterG_A=optimizer_B_2, updaterG_B=optimizer_A_2, lambda_cycle=1, discriminator =  ATACdiscriminator, cuda=True, ISRNA=False, sc_atac_test_dataset = sc_atac_test_dataset, sc_rna_test_dataset = sc_rna_test_dataset)
