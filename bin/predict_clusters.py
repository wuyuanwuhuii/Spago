...
# sc_rna_train_dataset_save=ad.read_h5ad('D:\\project\\scACT-main\\scACT-main\\data\\21623148\\MISAR_seq_mouse_E15_brain_data\\MISAR_seq_mouse_E15_brain_mRNA_data.h5ad')
# sc_atac_train_dataset_save=ad.read_h5ad('D:\\project\\scACT-main\\scACT-main\\data\\21623148\\MISAR_seq_mouse_E15_brain_data\\MISAR_seq_mouse_E15_brain_ATAC_data.h5ad')
...
def split_graph_data_atac(graph_rna, graph_atac, test_cluster, test_size=0.1, val_size=0.2):


    atac_clusters = graph_atac.ATAC_clusters
    test_mask = atac_clusters == test_cluster
    test_idx = torch.where(test_mask)[0]

    #
    train_val_mask = atac_clusters != test_cluster
    train_val_idx = torch.where(train_val_mask)[0]
    
    # 
    train_idx, val_idx = train_test_split(train_val_idx.numpy(), test_size=val_size / (1 - test_size), random_state=42)
...
def split_graph_data_rna(graph_rna, graph_atac, test_cluster, test_size=0.1, val_size=0.2):


    atac_clusters = graph_rna.RNA_clusters
    test_mask = atac_clusters == test_cluster
    test_idx = torch.where(test_mask)[0]
...
def split_graph_data_atac_2(graph_rna, graph_atac, test_cluster, test_size=0.1, output_csv='mask_info.csv'):
    ...
    other_cluster_mask = atac_clusters == 6  # 
    other_cluster_idx = torch.where(other_cluster_mask)[0]
    
 
    half_other_cluster_idx = other_cluster_idx[:len(other_cluster_idx) // 4]
    
  
    test_idx = torch.cat([test_idx, half_other_cluster_idx])

  
    train_mask = torch.ones(len(atac_clusters), dtype=torch.bool)  
    train_mask[test_idx] = False  
    train_idx = torch.where(train_mask)[0]  
    
   
    mask_data = {
        'spot_id': torch.arange(len(atac_clusters)).numpy(),  
        'train_mask': train_mask.numpy(),
        'test_mask': torch.zeros(len(atac_clusters), dtype=torch.bool).numpy()  
    }
    
    
    mask_data['test_mask'][test_idx.numpy()] = True
    ...
    return train_rna, train_atac, test_rna, test_atac
...
def preprocess_combined_graph_atac(adata, feature_type, k=14, metric='cosine'):
    coordinates = torch.tensor(list(zip(adata.obs['x'], adata.obs['y'])), dtype=torch.float32)
    #coordinates = torch.tensor(list(zip(adata.obs['X'], adata.obs['Y'])), dtype=torch.float32)
    nbrs = NearestNeighbors(n_neighbors=k + 1, algorithm='ball_tree').fit(coordinates)
    distances, indices = nbrs.kneighbors(coordinates)
    ...
    if feature_type == 'rna':
        ...
        raise ValueError("Invalid feature_type. Use 'rna' or 'atac'.")
    print(adata.obs['ATAC_clusters'].values)
    atac_clusters = adata.obs['ATAC_clusters'].str.replace('A', '').astype(int).values
    atac_clusters = torch.tensor(atac_clusters, dtype=torch.long)
    # 
    return Data(x=features, edge_index=edge_index_spatial, coordinates=coordinates, ATAC_clusters=atac_clusters)


def preprocess_combined_graph_rna(adata, feature_type, k=14, metric='cosine'):
    #coordinates = torch.tensor(list(zip(adata.obs['x'], adata.obs['y'])), dtype=torch.float32)
    coordinates = torch.tensor(list(zip(adata.obs['X'], adata.obs['Y'])), dtype=torch.float32)
    nbrs = NearestNeighbors(n_neighbors=k + 1, algorithm='ball_tree').fit(coordinates)
    distances, indices = nbrs.kneighbors(coordinates)
    ...
    if feature_type == 'rna':
        ...
        raise ValueError("Invalid feature_type. Use 'rna' or 'atac'.")
    print(adata.obs['RNA_clusters'].values)
    rna_clusters = adata.obs['RNA_clusters'].str.replace('R', '').astype(int).values
    print("ddddd")
    print(rna_clusters)
    
    rna_clusters = torch.tensor(rna_clusters, dtype=torch.long)
    # 
    return Data(x=features, edge_index=edge_index_spatial, coordinates=coordinates, RNA_clusters=rna_clusters)
...
def predict_atac(truth,generator,truth_iter, outdir_name, rna2atac, atac2rna, data_atac_save):
    ...
    data_atac_save.X = tt
    data_atac_save.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/E15_brain/predict_atac_ours_final.h5ad")
...
def predict_rna(truth,generator,truth_iter, outdir_name, rna2atac, atac2rna, data_atac_save):
    ...
    data_atac_save.X = tt
    data_atac_save.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/E15_brain/predict_rna_ours_A67.h5ad")
    print("8888888")
    fig = plot_scatter_with_r(
              truth,
              sc_rna_truth_preds,
...
def main():
    ...
    #sc_rna_train_dataset=ad.read_h5ad('D:\\project\\scACT-main\\scACT-main\\data\\21623148\\MISAR_seq_mouse_E15_brain_data\\MISAR_seq_mouse_E15_brain_mRNA_data.h5ad')
    #sc_atac_train_dataset=ad.read_h5ad('D:\\project\\scACT-main\\scACT-main\\data\\21623148\\MISAR_seq_mouse_E15_brain_data\\MISAR_seq_mouse_E15_brain_ATAC_data.h5ad')
    sc_rna_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/E15_brain/MISAR_seq_mouse_E15_brain_mRNA_data.h5ad')
    sc_atac_train_dataset=ad.read_h5ad('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/E15_brain/MISAR_seq_mouse_E15_brain_ATAC_data.h5ad')
    
    
    print(sc_rna_train_dataset)
    # sc_atac_train_dataset.obs['x'] = sc_rna_train_dataset.obs['X']
    # sc_atac_train_dataset.obs['y'] = sc_rna_train_dataset.obs['Y']
    ...
    print(np.max(sc_atac_train_dataset.X))
    # ATAC
    #
    ...
    data_rna_save = sc_rna_train_dataset
    print(sc_rna_train_dataset)
    ...
    data_atac_save = sc_atac_train_dataset
    print(data_atac_save.obs_names)
    

    all_atac_data = sc_atac_train_dataset
    ...
    # Split data
    #train_rna, train_atac, test_rna, test_atac = split_graph_data_atac_2(
    #graph_rna, 
    #graph_atac, 
    #test_cluster=5,  
    #test_size=0.1  
    ...
    #val_rna = train_rna
    #val_atac = train_atac
    #print(train_atac.ATAC_clusters)
    #print(test_atac.ATAC_clusters)
    #print(val_atac.ATAC_clusters)
    ...
    valid_dataset = PairedGraphDataset(val_rna, val_atac)
    ...
    atac2rna = both_GAN_1_new.AffineTransform(16, 256, affine_num=9)
    ...
    for h_dim, lr, rand_seed in param_combos:
        ...
        def train_vgae(model, optimizer, data, epochs, device, truth_atac, discriminator, updaterD):
            ...
            for epoch in range(epochs):
                ...
                loss2 = loss_D(y, y_hat, discriminator, data.edge_index, data.coordinates.to(device))
        ...
        def train_cycle_consistency(generator_A, generator_B, num_epochs, train_iter, test_iter,truth_iter,
                            updaterG_A, updaterG_B, lambda_cycle, discriminator, cuda=True, ISRNA=True, data_atac_save=data_atac_save, data_rna_save=data_rna_save):
            # 
            device_rna = 'cuda:0'
            device_atac = 'cuda:0'
            ...
            val_atac.to(device_rna)
            val_rna.to(device_rna)
            ...
            for epoch in range(num_epochs):
                ...
                for i in range(1):
                    ...
                    if(ISRNA) :
                        ...
                        rna_recon_cycle_loss = loss_rna(preds=retval1, theta=retval2, truth=rna_features,)
                        #rna_recon_cycle_loss, recon, kl = generator_B.compute_elbo(mu_atac, logstad_atac, rna_recon_cycle_loss_1,)
                    ...
                    if ISRNA:
                        ...
                        loss_A = rna_recon_cycle_loss * 10  + 0.01 * (cycle_loss_rna) #+ rna_recon_cycle_loss_train # 10 + reg_loss_rna * 0.01 + lambda_cycle * (cycle_loss_rna )#+ cycle_loss_atac) # + kl_rna
                    else :
                        #loss_A = atac_recon_cycle_loss * 0.01 + reg_loss_atac  + lambda_cycle * ( cycle_loss_atac) #+ kl_atac
                        loss_A =  atac_recon_cycle_loss #* 100 + cycle_loss_atac * 0.01 #1 * ( cycle_loss_atac) + atac_recon_cycle_loss_train *  0.01# + reg_loss_atac * 0.01 + lambda_cycle * ( cycle_loss_atac) #+ kl_atac
                ...
                if ((epoch + 1) % 10== 0):
                    if ISRNA:
                        truth = graph_rna.x#.to(device)
                        predict_rna(truth, generator_B, graph_atac, outdir_name,  rna2atac, atac2rna, data_rna_save)
                    else:
                        truth = graph_atac.x#.to(device)
                        predict_atac(truth, generator_B, graph_rna, outdir_name, rna2atac, atac2rna, data_atac_save)
                # 
                ...
                if test_iter:
                    ...
                    with torch.no_grad():
                        ...
                        if ISRNA:
                            ...
                            loss_test = rna_recon_cycle_loss_test * 10 + 0.01 * (rna_latent_recon_test) #+ rna_recon_cycle_loss_test_2  #* 10 + reg_loss_rna * 0.01 + lambda_cycle * (cycle_loss_rna ) #  + kl_rna
                        else :
                            #loss_test = atac_recon_cycle_loss  + reg_loss_atac + lambda_cycle * (cycle_loss_atac) #  + kl_atac
                            loss_test = atac_recon_cycle_loss_test #* 100 + atac_latent_recon_test * 0.01 # 1 * (cycle_loss_atac_test) + atac_recon_cycle_loss_test_2 *  0.01 #* 0.1 + reg_loss_atac * 0.01 + lambda_cycle * (cycle_loss_atac) #  + kl_atac
                    ...
                    if early_stopping.early_stop:
                        ...
                        if ISRNA:
                            truth = graph_rna.x#.to(device_rna)
                            predict_rna(truth, generator_B, graph_atac, outdir_name, rna2atac, atac2rna, data_rna_save)
                        else:
                            truth = graph_atac.x#.to(device_rna)
                            predict_atac(truth, generator_B, graph_rna, outdir_name, rna2atac, atac2rna, data_atac_save)
                        break
        ...
        # test_iter=test_loader,truth_iter = truth_loader,updaterG_A=optimizer_A, updaterG_B=optimizer_B, lambda_cycle=0.01, discriminator = RNAdiscriminator, cuda=True, ISRNA=True,data_atac_save=data_atac_save, data_rna_save=data_rna_save)
        ...
        test_iter=test_loader,truth_iter = truth_loader,updaterG_A=optimizer_B_2, updaterG_B=optimizer_A_2, lambda_cycle=1, discriminator =  ATACdiscriminator, cuda=True, ISRNA=False, data_atac_save=data_atac_save, data_rna_save=data_rna_save)
