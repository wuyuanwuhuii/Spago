...
def to_dense_tensor(adata):
    from scipy import sparse
    adata.X = sparse.coo_matrix(adata.X)
    Acoo = adata.X.tocoo()
    tensor = scipy_sparse_mat_to_torch_sparse_tensor(Acoo).to_dense()
    return tensor
...
def main():
    ...
    train_rna = sc.read('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/adata_RNA_1.h5ad')
    train_pro = sc.read('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/adata_ADT_1.h5ad')
    
    
    #train_pro.X = clr_transform(train_pro.X)

    # RNA 
    #adata_temp = train_rna.copy()
    ...
    # 
    #sc.pp.highly_variable_genes(adata_temp, n_top_genes=3000)
    
    # 
    #highly_variable_genes = adata_temp.var['highly_variable']
    #train_rna = train_rna[:, adata_temp.var['highly_variable']].copy()
    
    train_pro_save = train_pro
    
    #sc.pp.normalize_total(train_rna)
...
# Model
    ...
    for h_dim,lr,rand_seed in param_combos:
        ...
        GeneratorProtein = both_GAN_1.GeneratorProtein(hidden_dim=h_dim,
                                           input_dim1=train_rna.shape[1],
                                           input_dim2=train_pro.shape[1],
        ...
        DiscriminatorProtein = both_GAN_1.DiscriminatorProtein(input_dim=train_pro.shape[1],seed=rand_seed)
        ...
        def predict_protein_2(truth,generator,truth_iter):
            ...
            def predict1(generator,truth_iter):
                ...
                with torch.no_grad():  # 禁止梯度的计算
                    x_np = truth_iter.X.toarray() if sparse.issparse(truth_iter.X) else truth_iter.X
                    x_tensor = torch.tensor(x_np, dtype=torch.float32)
                    if torch.cuda.is_available():
                        x_tensor = x_tensor.cuda()
                    ret = generator(x_tensor)
                #        y_pred = generator(x)
                #        if first == 1:
                #            ret = y_pred
                #            first = 0
            ...
            print(truth.shape)
            print(sc_rna_protein_truth_preds.shape)
            
            
            if isinstance(sc_rna_protein_truth_preds, torch.Tensor):
                tt = sc_rna_protein_truth_preds.cpu().numpy()  # 
            
            truth_r = truth.X.toarray() if sparse.issparse(truth.X) else truth.X
            #train_pro_save.X = tt
            #train_pro_save.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/predict_pro_scMOG_ADT2.h5ad")
        ...
        def predict_protein(truth,generator,truth_iter):
            ...
            #truth_r = truth.X.toarray() if sparse.issparse(truth.X) else truth.X
            #train_pro_save.X = tt
            #train_pro_save.write_h5ad("/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/predict_pro_scMOG.h5ad")
            fig = plot_scatter_with_r(
                truth,
                sc_rna_protein_truth_preds,
                one_to_one=True,
        ...
        sc_rna_train_dataset=sc.read('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/adata_RNA_1.h5ad')
        sc_rna_test_dataset =sc.read('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/adata_RNA_1.h5ad')
        sc_atac_train_dataset=sc.read('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/adata_ADT_1.h5ad')
        sc_atac_test_dataset=sc.read('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/adata_ADT_1.h5ad')
        sc_rna_truth_dataset=sc.read('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/adata_RNA_1.h5ad')
        sc_atac_truth_dataset=sc.read('/mnt/datab/home/zhaohui/WBT/scMOG-main/scMOG_code/spleen1_protein/adata_ADT_1.h5ad')
        print('rrrrrr')
        print(sc_rna_train_dataset)
        
        sc_atac_train_dataset.X = clr_transform(sc_atac_train_dataset.X)
        sc_atac_test_dataset.X = clr_transform(sc_atac_test_dataset.X)
        sc_atac_truth_dataset.X = clr_transform(sc_atac_truth_dataset.X)
        
        sc.pp.normalize_total(sc_rna_train_dataset)
        sc.pp.log1p(sc_rna_train_dataset)
        
        sc.pp.normalize_total(sc_rna_test_dataset)
        sc.pp.log1p(sc_rna_test_dataset)
        
        sc.pp.normalize_total(sc_rna_truth_dataset)
        sc.pp.log1p(sc_rna_truth_dataset)
        
        #sc_rna_train_dataset = train_rna
        #sc_rna_test_dataset = train_rna
        #sc_atac_train_dataset = train_pro
        #sc_atac_test_dataset = train_pro
        #sc_rna_truth_dataset = train_rna
        #sc_atac_truth_dataset = train_pro
        
        sc_rna_train = to_dense_tensor(sc_rna_train_dataset)
        sc_atac_train = to_dense_tensor(sc_atac_train_dataset)
        sc_rna_test = to_dense_tensor(sc_rna_test_dataset)
        sc_atac_test = to_dense_tensor(sc_atac_test_dataset)
        sc_rna_truth = to_dense_tensor(sc_rna_truth_dataset)
        sc_atac_truth = to_dense_tensor(sc_atac_truth_dataset)
        
        
        sc_rna_train_dataset.X = sparse.coo_matrix(sc_rna_train_dataset.X) 
        ...
        sc_atac_train_dataset.X = sparse.coo_matrix(sc_atac_train_dataset.X)
        ...
        sc_rna_test_dataset.X = sparse.coo_matrix(sc_rna_test_dataset.X)
        ...
        sc_atac_test_dataset.X = sparse.coo_matrix(sc_atac_test_dataset.X)  
        ...
        Acoo5 = sc_rna_truth_dataset.X.tocoo()
        sc_rna_truth = scipy_sparse_mat_to_torch_sparse_tensor(Acoo5)
        ...
        truth_iter_rna = torch.utils.data.DataLoader(sc_rna_truth, batch_size=64)
        
       
        
        #GeneratorProtein.load_state_dict(torch.load(os.path.join(outdir_name, "Proteingenerator.pth")))
        #predict_protein_2(sc_atac_truth_dataset, GeneratorProtein, sc_rna_train_dataset)
        #ddddd
